...
static unsigned short normal_addr[] = { 0x74, I2C_CLIENT_END };
...
struct i2c_client *system_client;


u8 pca9539_send (u8 in_addr, u8 val)
{
	u8 ret;
//	printk("%s: addr(0x%x), val(0x%x) \n",__FUNCTION__,in_addr,val);
	ret = i2c_smbus_write_byte_data(system_client,in_addr, val);
	return ret;
}
EXPORT_SYMBOL(pca9539_send);

u8 pca9539_recv (u8 in_addr)
{
	u8 ret;
//	printk("%s: addr(0x%x)\n",__FUNCTION__,in_addr);
	ret = i2c_smbus_read_byte_data(system_client,in_addr);
	return ret;
}
EXPORT_SYMBOL(pca9539_recv);
...
//	printk("%s : flag(%d),addr(0x%x),index(0x%x)\n",__FUNCTION__,client->flags,client->addr,psa->index);
...
//	printk("%s : flag(%d),addr(0x%x),index(0x%x),val(0x%x)\n",__FUNCTION__,client->flags,client->addr,psa->index,val);
