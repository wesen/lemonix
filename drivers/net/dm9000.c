...
/*
 * CSUNG added
 * Ethernet DM9000B Chipselect
 */
#define DM9000B_CHIPSELECT	0x80000000
...
dm9000_probe(struct platform_device *pdev)
{
	...
	dmfe_dev    = ndev;
	iosize = res_size(addr_res);
	addr_req = request_mem_region(addr_res->start, iosize,
			pdev->name);
	...
	db->io_addr = ioremap(addr_res->start, iosize);
	...
	iosize = res_size(data_res);
	data_req = request_mem_region(data_res->start, iosize,
			pdev->name);

	if (data_req == NULL) {
		printk("cannot claim data reg area\n");
		ret = -EIO;
		goto out;
	}

	db->io_data = ioremap(data_res->start, iosize);

	if (db->io_data == NULL) {
		printk("failed to ioremap data reg\n");
		ret = -EINVAL;
		goto out;
	}
	...
	iobase =  (unsigned long)db->io_addr;
	...
	if (err)
	{
		printk("register_netdev error %d \n",err);
		goto out1;
	}
	return 0;

out1:
	release_region(ndev->base_addr,4);
out:
	free_netdev(ndev);
	return ERR_PTR(err);
	return 0;
...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	//	if (!request_mem_region(iophybase, 8, "dm9000"))
	//		return -EBUSY;
	//	iobase = ioremap(iophybase, 8);
...
static int dmfe_open(struct net_device *dev)
{
	...
	smr = at91_sys_read(AT91_AIC_SMR(dev->irq)) & ~AT91_AIC_SRCTYPE;
	at91_sys_write(AT91_AIC_SMR(dev->irq), smr | AT91_AIC_SRCTYPE_HIGH);
...
static struct resource dm9000b_resource[] = {
	[0] = {
		.start	= DM9000B_CHIPSELECT,
		.end	= DM9000B_CHIPSELECT + 3,
		.flags	= IORESOURCE_MEM,
	},
	[1] = {
		.start	= DM9000B_CHIPSELECT + 0x44,
		.end	= DM9000B_CHIPSELECT + 0xff,
		.flags	= IORESOURCE_MEM
	},
	[2] = {
		.start	= AT91SAM9260_ID_IRQ1,
		.end	= AT91SAM9260_ID_IRQ1,
...
static struct platform_device dm9000b_device = {
	.name		= "dm9000",
	.id		= 0,
	.num_resources	= ARRAY_SIZE(dm9000b_resource),
	.resource	= dm9000b_resource,
	.dev		= {
...
dm9000_init(void)
{
	...
	at91_sys_write(AT91_SMC_SETUP(7), AT91_SMC_NWESETUP_(8) | AT91_SMC_NCS_WRSETUP_(4) | AT91_SMC_NRDSETUP_(8) | AT91_SMC_NCS_RDSETUP_(4));
	at91_sys_write(AT91_SMC_PULSE(7), AT91_SMC_NWEPULSE_(8) | AT91_SMC_NCS_WRPULSE_(16) | AT91_SMC_NRDPULSE_(8) | AT91_SMC_NCS_RDPULSE_(16));
	at91_sys_write(AT91_SMC_CYCLE(7), AT91_SMC_NWECYCLE_(24) | AT91_SMC_NRDCYCLE_(24));
	at91_sys_write(AT91_SMC_MODE(7), AT91_SMC_READMODE | AT91_SMC_WRITEMODE | AT91_SMC_EXNWMODE_DISABLE | AT91_SMC_BAT_WRITE | AT91_SMC_DBW_16 | AT91_SMC_TDF_(2));
	/* Configure Reset signal as output */
	at91_set_B_periph(AT91_PIN_PC12, 0);	/* NCS7 */

	/* Configure Interrupt pin as input, no pull-up */
	at91_set_B_periph(AT91_PIN_PC15, 0);	/* IRQ1 */

	platform_device_register(&dm9000b_device);
