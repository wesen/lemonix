...
static struct kvm *kvm_create_vm(void)
{
	...
	for (i = 0; i < KVM_MAX_VCPUS; ++i) {
		...
		vcpu->cpu = -1;
		spin_lock_init(&vcpu->hyper_vcpu.lock);
		...
		spin_lock_init(&kvm->vcpus[i].irq_lock);
	}

	kvmnet_host_init(kvm);
	return kvm;
}
 ...
 */
int __load_pdptrs(struct kvm_vcpu *vcpu, unsigned long cr3)
{
...
EXPORT_SYMBOL_GPL(__load_pdptrs);
...
EXPORT_SYMBOL_GPL(kvm_cr3_cache_clear);
...
int hypercall_test(struct kvm_vcpu *vcpu)
{
...
int hypercall_trace(unsigned long caller, unsigned long v1, unsigned long v2,
		    unsigned long v3)
{
	__trace_special(caller, v1, v2, v3);

	return 0;
}

static void * hypercall_table[] =
{
	[__NR_hypercall_load_cr3] =
			(void *) hypercall_load_cr3,
	[__NR_hypercall_flush_cr3_cache] =
			(void *) hypercall_flush_cr3_cache,
	[__NR_hypercall_get_ktime] =
			(void *) hypercall_get_ktime,
	[__NR_hypercall_register_eth] =
			(void *) hypercall_register_eth,
	[__NR_hypercall_send_eth] =
			(void *) hypercall_send_eth,
	[__NR_hypercall_test] =
			(void *) hypercall_test,
	[__NR_hypercall_disk_read] =
			(void *) hypercall_disk_read,
	[__NR_hypercall_disk_write] =
			(void *) hypercall_disk_write,
	[__NR_hypercall_disk_register_completion_area] =
			(void *) hypercall_disk_register_completion_area,
	[__NR_hypercall_disk_register_submit_area] =
			(void *) hypercall_disk_register_submit_area,
	[__NR_hypercall_disk_trigger_scan] =
			(void *) hypercall_disk_trigger_scan,
	[__NR_hypercall_disk_info] =
			(void *) hypercall_disk_getinfo,
	[__NR_hypercall_disk_count] =
			(void *) hypercall_disk_getdiskcount,
	[__NR_hypercall_trace] =
			(void *) hypercall_trace,
};
...
int kvm_hypercall(struct kvm_vcpu *vcpu, struct kvm_run *run)
{
...
#endif
	{
        ...
        if (unlikely(prof_on == KVM_PROFILING && nr < __NR_hypercalls))
                profile_hit(KVM_PROFILING, hypercall_table[nr]);

	switch (nr) {
	case __NR_hypercall_load_cr3:
		ret = hypercall_load_cr3(vcpu, a0);
		break;
	case __NR_hypercall_flush_cr3_cache:
		ret = hypercall_flush_cr3_cache(vcpu, a0);
		break;
	case __NR_hypercall_get_ktime:
		ret = hypercall_get_ktime(vcpu, a0);
		break;
	case __NR_hypercall_register_eth:
		ret = hypercall_register_eth(vcpu, a0);
		break;
	case __NR_hypercall_send_eth:
		ret = hypercall_send_eth(vcpu);
		break;
	case __NR_hypercall_test:
		ret = hypercall_test(vcpu);
		break;
	case __NR_hypercall_disk_read:
		ret = hypercall_disk_read(vcpu, a0, a1, a2, a3);
		break;
	case __NR_hypercall_disk_write:
		ret = hypercall_disk_write(vcpu, a0, a1, a2, a3);
		break;
	case __NR_hypercall_disk_register_completion_area:
		ret = hypercall_disk_register_completion_area(vcpu, a0, a1, a2);
		break;
	case __NR_hypercall_disk_register_submit_area:
		ret = hypercall_disk_register_submit_area(vcpu, a0, a1, a2);
		break;
	case __NR_hypercall_disk_trigger_scan:
		ret = hypercall_disk_trigger_scan(a0);
		break;
	case __NR_hypercall_disk_info:
		ret = hypercall_disk_getinfo(vcpu, a0, a1);
		break;
	case __NR_hypercall_disk_count:
		ret = hypercall_disk_getdiskcount();
		break;
	case __NR_hypercall_trace:
		ret = hypercall_trace(a0, a1, a2, a3);
		break;
	default:
		WARN_ON_ONCE(1);
	}
...
static int kvm_vcpu_ioctl_exec(struct kvm_vcpu *vcpu, struct kvm_run *kvm_run)
{
	int r;
				    ...
				    struct kvm_sregs *sregs)
{
	...
	spin_lock_bh(&vcpu->irq_lock);
	memcpy(sregs->interrupt_bitmap, vcpu->irq_pending,
	       sizeof sregs->interrupt_bitmap);
	spin_unlock_bh(&vcpu->irq_lock);
...
int kvm_inject_irq(struct kvm_vcpu *kvm_vcpu, struct kvm_interrupt *irq)
{
	if (irq->irq < 0 || irq->irq >= 256)
		return -EINVAL;

	spin_lock_bh(&kvm_vcpu->irq_lock);
	set_bit(irq->irq, kvm_vcpu->irq_pending);
	set_bit(irq->irq / BITS_PER_LONG, &kvm_vcpu->irq_summary);
	spin_unlock_bh(&kvm_vcpu->irq_lock);
			   ...
			   unsigned int ioctl, unsigned long arg)
{
	...
	switch (ioctl) {
	case KVM_BIND_VCPU:
		r = hyper_bind_vcpu(current, &vcpu->hyper_vcpu, vcpu);
		break;

	case KVM_UNBIND_VCPU:
		r = hyper_unbind_vcpu(current);
		break;
	...
	case KVM_EXEC:
	{
		...
		r = kvm_vcpu_ioctl_exec(vcpu, &kvm_run);
		if (r < 0 &&  r != -EINTR)
			goto out;
