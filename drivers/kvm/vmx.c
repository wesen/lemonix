...
static void kvm_do_inject_irq(struct kvm_vcpu *vcpu)
{
	...
	irq = word_index * BITS_PER_LONG + bit_index;
...
static int handle_wrmsr(struct kvm_vcpu *vcpu, struct kvm_run *kvm_run)
{
	...
	printk(KERN_INFO "wrmsr, ecx: %08x, data: %08Lx, eip: %08lx\n",
		ecx, data, vmcs_readl(GUEST_RIP));
...
static int kvm_handle_exit(struct kvm_run *kvm_run, struct kvm_vcpu *vcpu)
{
	...
	/*
	 * Profile KVM exit RIPs - except vmcalls that get profiled
	 * in a more finegrained way, separately:
	 */
