...
struct kvm_vcpu {
	struct hyper_vcpu hyper_vcpu;
	struct kvm *kvm;
	union {
		struct vmcs *vmcs;
		struct vcpu_svm *svm;
	};
	spinlock_t lock; /* use hyper_vcpu.lock instead */
	int   cpu;
	int   launched;
...
hpa_t __gpa_to_hpa(struct kvm *vcpu, gpa_t gpa);
...
int hypercall_trace(unsigned long caller, unsigned long v1, unsigned long v2,
		    unsigned long v3);
int hypercall_flush_cr3_cache(struct kvm_vcpu *vcpu, unsigned long old_cr3);
int hypercall_register_eth(struct kvm_vcpu *vcpu, gpa_t eth_state_gpa);
int hypercall_send_eth(struct kvm_vcpu *vcpu);
int hypercall_disk_read(struct kvm_vcpu *vcpu, int nr, unsigned long start_sector,
...
int hypercall_disk_register_completion_area(struct kvm_vcpu *vcpu, int nr,
              unsigned long completion_page_gpa, int irq);
...
int kvmnet_host_init(struct kvm*);
void kvmnet_host_exit(struct kvm*);
