...
hpa_t __gpa_to_hpa(struct kvm *kvm, gpa_t gpa)
{
	struct kvm_memory_slot *slot;
	struct page *page;
...
EXPORT_SYMBOL_GPL(__gpa_to_hpa);

inline hpa_t gpa_to_hpa(struct kvm_vcpu *vcpu, gpa_t gpa)
{
       return __gpa_to_hpa(vcpu->kvm, gpa); 
}
...
/*
 * Pagetable is freed by the guest, flush the cr3 cache:
 */
