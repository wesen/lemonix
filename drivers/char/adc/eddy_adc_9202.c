...
static struct at91_adc_channel adc_channel;
static struct at91_adc_value adc_value;
 ...
 */
static int at91_adc_open(struct inode *inode, struct file *file)
{
 ...
 */
static int at91_adc_close(struct inode *inode, struct file *file)
{
 ...
 */
static ssize_t at91_adc_write(struct file *file, const char *data, size_t len, loff_t *ppos)
{
	return 0;
}
...
static int __init at91adc_probe(struct platform_device *pdev)
{
	...
	if (!request_mem_region(adc_channel.mapbase, size, "at91_adc"))
		return -EBUSY;
...
{
	int ret = 0;
	at91_set_A_periph(AT91_PIN_PC0, 0);		/* TWD */
	at91_set_A_periph(AT91_PIN_PC1, 0);		/* TWCK */
#ifdef ADC_TIMER
