...
static int wdt_timeout = -1;		/* default timeout = 3 sec */

int reload_flag = 3;
int time_flag = 1; 

static struct timer_list wdt_timer;
#define WDT_RELOAD_DELAY (1000 * HZ / 1000) /* every 1 sec */
...
static int at91_wdt_settimeout(int new_time)
{
	...
	if(mtd_id == 0x7f)
	{
		...
		reg = AT91_WDT_WDRSTEN					/* causes watchdog reset */
			| AT91_WDT_WDRPROC				/* causes processor reset if useing this register, it will effect to flash functions. */
	...
	printk(KERN_INFO "AT91SAM9 Watchdog enabled (%d seconds, nowayout = 0x%x )\n", wdt_timeout,mr);
	return 0;
}

static void wdt_reloading_timer (unsigned long unused)
{  
	at91_wdt_reload();
	mod_timer(&wdt_timer, jiffies + WDT_RELOAD_DELAY);
	if(reload_flag == 0 && time_flag == 1)
	{
		time_flag = 0;
		wdt_timeout = 3;
...
static int __init at91sam_wdt_init(void)
{
	...
	wdt_timer.function = wdt_reloading_timer;
	mod_timer(&wdt_timer, jiffies + WDT_RELOAD_DELAY);
