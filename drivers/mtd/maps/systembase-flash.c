...
#define BOARD_FLASH_SIZE_8  0x00800000 /* 8MB */
...
int __init systembase_mtd_init(void)
{
	...
	systembase_map_8.bankwidth = BOARD_FLASH_WIDTH;

	window_addr = 0x10000000; 
	window_size = BOARD_FLASH_SIZE_8;
	 ...
	 */
	parts = systembase_partitions_8;
	nb_parts = ARRAY_SIZE(systembase_partitions_8);
	systembase_map_8.size = window_size;
	...
	//		systembase_map_8.bankwidth*8);
	systembase_map_8.virt = ioremap(window_addr, window_size);
	mymtd = do_map_probe("jedec_probe", &systembase_map_8);
	if (!mymtd) {
		iounmap(systembase_map_8.virt);
		return -ENXIO;
	}
