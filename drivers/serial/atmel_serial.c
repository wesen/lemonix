...
int PDC_BUFFER_SIZE	 = L1_CACHE_BYTES;
...
static void atmel_stop_tx(struct uart_port *port)
{
	...
	tmp_count = quot_num * 100;

	if (atmel_port->use_dma_tx) 
	{
		UART_PUT_PTCR(port, ATMEL_PDC_TXTDIS);		/* disable PDC transmit */
		UART_PUT_IDR(port, ATMEL_US_ENDTX | ATMEL_US_TXBUFE);
		if(port->interface_type == RS485NE)
		{
			CSR_value = UART_GET_CSR(port); //shlee_7907

			while(!(CSR_value & ATMEL_US_TXEMPTY) && tmp_count-- )
			{
			...
			{
				at91_set_gpio_value(AT91_PIN_PC21,GPIO_LOW);
				udelay(1);
				at91_set_gpio_value(AT91_PIN_PC22,GPIO_LOW);
			}
	...
	if (atmel_port->use_dma_tx) 
	{
		...
		if(port->interface_type == RS485NE)
		{
			if(port->line == 1)
			{
				...
				udelay(1);
				at91_set_gpio_value(AT91_PIN_PC18,GPIO_HIGH);
				udelay(1);
			}
			else if(port->line == 2)
			{
				at91_set_gpio_value(AT91_PIN_PC21,GPIO_HIGH);
				udelay(1);
				at91_set_gpio_value(AT91_PIN_PC22,GPIO_HIGH);
				udelay(1);
			}
		}
...
static void atmel_set_termios(struct uart_port *port, struct ktermios * termios, struct ktermios * old)
{
	...
	if(baud >= 57600 )
	{
		PDC_BUFFER_SIZE = 4096 / (921600 / baud);
		port->fifosize	= PDC_BUFFER_SIZE;
	}
	else 
	{	
		PDC_BUFFER_SIZE = 256 / (38400 / baud);
		port->fifosize	= PDC_BUFFER_SIZE;	
	}
	...
	//-->shlee_8326
	// re- initial the PDC because PDC_BUFFER_SIZE can be changed
...
static void __devinit atmel_init_port(struct atmel_uart_port *atmel_port, struct platform_device *pdev)
{
	...
	if(port->line == 3 || port->line == 4)
	{
		atmel_port->Autotoggle = AUTO;
	}
	else
	{
		atmel_port->Autotoggle = MANUAL;
	}
