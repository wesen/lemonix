	...
	switch (cmd) {
		...
		case TIOTGTXCNT:
			...
			if(circ->head == circ->tail)
				ret_buf_size = 1024;
			else if(circ->head > circ->tail)
				ret_buf_size = (( 1024 - circ->head ) + circ->tail);
			else if(circ->head < circ->tail)
				ret_buf_size = (circ->tail - circ->head );
			...
			if(circ->head == circ->tail)
				...
				ret_buf_size = (( 2046 - circ->head ) + circ->tail);
			else if(circ->head < circ->tail)
				ret_buf_size = (circ->tail - circ->head );

			if(ret_buf_size > 1500)
				ret = 1024;
			else
				ret = 0;
			return ret;
		...
		case INTERFACESEL:
			...
			//	printk("interface select = %d eddyID = %d \n",tmp,eddyID);
				switch(tmp)
				{
					case RS232:
						/* rs232 on */
						//		at91_set_gpio_output(AT91_PIN_PC19,GPIO_HIGH);
						port->interface_type = RS232;
						break;
					case RS422PTP:

						if(eddyID & 0x20)
						{
							if (eddyID == EDDY_S2_M_CA)
							{
								if(port->line == 1)
								{
									/* receive enable */
...
int get_product(void)
{
	...
	for(gpio_num = AT91_PIN_PC6,val_gpio = 0 ; gpio_num <= AT91_PIN_PC10 ; gpio_num++)
	{
		...
		udelay(1);
		eddyID |= ( val_gpio << ( gpio_num - AT91_PIN_PC6));
	}
	
	if(eddyID < 0x10 && eddyID & 0x1) //--> 
	{
		// only for EDDY_S1_DB9_C, EDDY_S1_PIN_C, EDDY_S2M_PIN_C, EDDY_CNVT_C
		// EDDY_DK_C is not supported.
		at91_set_gpio_input(AT91_PIN_PC18,PULLUP_ENABLE);
		udelay(1);
		val_gpio = at91_get_gpio_value(AT91_PIN_PC18);
		// only EDDY_S1_DB9_CA, EDDY_S1_PIN_CA, EDDY_S2M_PIN_CA, EDDY_CNVT_CA 
		// product AT91_PIN_PC18 is high 
		eddyID |= ( val_gpio << ( gpio_num - AT91_PIN_PC6));	
		udelay(1);
	}
