...
static inline void __iomem *pin_to_controller(unsigned pin)
{
 ...
 EXPORT_SYMBOL(pin_to_controller);
...
/*
 *systembase make this function by shlee in 7830
 */

int __init_or_module at91_get_gpio_mode(unsigned pin)
{
	void __iomem	*pio = pin_to_controller(pin);
	unsigned	mask = pin_to_mask(pin);
	u32		posr = 0;
	...
	posr = __raw_readl(pio + PIO_OSR);
	return (posr & mask) != 0;

}
EXPORT_SYMBOL(at91_get_gpio_mode);
...
/*
 * shlee added for check registers
 */

int at91_get_gpio_register(unsigned pin)
{
	void __iomem	*pio = pin_to_controller(pin);
	unsigned	mask = pin_to_mask(pin);
	u32		pdsr;
	...
	while(i < 0xAC)
	{	
		pdsr = __raw_readl(pio + i);
		printk("(0x%x)    0x%x \n",i,pdsr);
		i += 0x4;
	}
	return 0;
}
EXPORT_SYMBOL(at91_get_gpio_register);

void __iomem * return_gpio_base_addr()
{
	void __iomem	*pio = pin_to_controller(AT91_PIN_PA0);
	return pio;
}
EXPORT_SYSBOL(return_gpio_base_addr);
